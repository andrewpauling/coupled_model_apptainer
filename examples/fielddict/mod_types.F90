...
!
   ...
   integer, parameter :: nATMExportList = 32
   character(ESMF_MAXSTR), dimension(1:nATMExportList) :: &
      atm_exportWRFNameList = &
      (/'LANDMASK', 'XLAT', 'XLONG', &
        'LH_SEAICE', 'HFX_SEAICE', &
        'SWNET_SEAICE', 'LWNET_SEAICE', &
        'LH_OCEAN', 'HFX_OCEAN', &
        'SWNET_OCEAN', 'LWNET_OCEAN', &
        'SWVISDIR', 'SWVISDIF', 'SWNIRDIR', 'SWNIRDIF', 'LWDNB', &
        'SFROFF', 'SFROFF_LAST', 'LAND_RUNOFF', &
        'U10', 'V10', 'T2', 'Q2', 'SFCEVP', 'QFX', 'RAINBL', &
        'HAILNCV', 'GRAUPELNCV', 'SNOWNCV', 'SNOWH_SI', 'DELTA_SNOW', &
        'SST_INPUT'/)

   integer, parameter :: nATMImportList = 9
   character(ESMF_MAXSTR), dimension(1:nATMImportList) :: &
      atm_importWRFNameList = &
      (/'SST', 'UOCE', 'VOCE', 'FRZMLT', 'MLD', &
        'SEAICE', 'SITICE', 'SISNOW', 'SIHEFF'/)

   integer, parameter :: nOCNExportList = 5
   character(ESMF_MAXSTR), dimension(1:nOCNExportList) :: &
      ocn_exportWRFNameList = &
      (/'SST', 'UOCE', 'VOCE', 'FRZMLT', 'MLD'/)

   integer, parameter :: nOCNImportList = 27
   character(ESMF_MAXSTR), dimension(1:nOCNImportList) :: &
      ocn_importWRFNameList = &
      (/'LANDMASK', 'XLAT', 'XLONG', &
   ...
   integer, parameter :: nICEExportList = 4
   character(ESMF_MAXSTR), dimension(1:nICEExportList) :: &
      ice_exportWRFNameList = &
      (/'SEAICE', 'SITICE', 'SISNOW', 'SIHEFF'/)

   integer, parameter :: nICEImportList = 20
   character(ESMF_MAXSTR), dimension(1:nICEImportList) :: &
      ice_importWRFNameList = &
      (/'SWVISDIR', 'SWVISDIF', 'SWNIRDIR', 'SWNIRDIF', 'LWDNB', &
        'U10', 'V10', 'T2', 'Q2', 'SFCEVP', 'QFX', 'RAINBL', 'HAILNCV', &
        'GRAUPELNCV', 'SNOWNCV', 'SST', 'UOCE', 'VOCE', 'FRZMLT', 'MLD'/)
!
